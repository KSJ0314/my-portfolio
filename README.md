![](https://velog.velcdn.com/images/ksj0314/post/3d4ca6cf-6e21-49b4-80ad-9106df5c148a/image.png)
...
* `clip-path: polygon()`을 이용하여 장면 전환 애니메이션을 구현하였습니다.
...
* 좌측에는 `react-bootstrap`의 `Carousel`을 이용하여 프로젝트의 이미지를 확인할 수 있습니다.
...
* 깃허브에 작성된 `ReadMe` 파일을 다운받아 프로젝트에 추가하고
`react-markdown`을 이용하여 마크다운 문법에 맞게 html에 렌더링하여 나타냅니다.
...
* `npm install firebase` 명령어를 이용해 Firebase를 설치하여
데이터의 저장, 조회, 삭제를 구현하였습니다.
* 아이디, 비밀번호, 작성일자, 방명록은 Firestore에 저장되며
...
* 확대될 때의 `transition` 효과를 위해 해당 영역을 감싸고 있는 컴포넌트의 `position` 값을 계산하여 하위 컴포넌트로 넘겨주고 하위 컴포넌트를 `position: fixed`로 설정하였습니다.
...
* 데스크탑 모드에서는 해당 영역의 컴포넌트가 변경되는 방식으로,
모바일 모드에서는 화면이 스크롤되는 방식으로 구현하였습니다.
* 페이지의 가로 세로 비율을 계산하여 데스크탑(가로) / 모바일(세로) 모드로 변경됩니다.
